...
export class CoreConfigService {
  ...
  constructor(private _router: Router, @Inject(CORE_CUSTOM_CONFIG) private _config) {
    ...
    if (_config.layout.enableLocalStorage) {
      this.localConfig = JSON.parse(localStorage.getItem('carental-config'));
    } else {
      localStorage.removeItem('carental-config');
    }
  ...
  set config(data) {
    ...
    if (config.layout.enableLocalStorage) {
      localStorage.setItem('carental-config', JSON.stringify(config));
    }
