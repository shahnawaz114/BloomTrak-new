    ...
    checkUniqueUser(userData, function (callbackres, deletedUser) {
        ...
        if (callbackres == false) {
            var insertData = {
                ...
                management_id: userData.management_id,
...
function mgCommunities(id) {
    return new Promise((resolve, reject) => {
        ...
        sql.query(`SELECT cp.id FROM community_portal as cp LEFT JOIN management as mm ON mm.id = cp.management_id WHERE mm.id = ?`, [id], (err, res) => {
            if (err) {
                reject({ err: true, data: [] })
            } else {
            ...
            WHERE ${searchStr} uu.is_deleted = '0' ORDER BY uu.created_at desc LIMIT ? OFFSET ?`, [limitNum, startNum], async (error, res) => {
                if (error) {
                    ...
                    if (userData.is_for && userData.is_for == 'management') {
                        ...
                        console.log("___________________________is_for agency and community")
                        finalResult = res.map(item => {
        ...
        if (error) {
            ...
            if (userData.is_for == 'agency') {
                allUsers = allUsers.filter(item => {
                    const agencyUsers = item.agency_id ? item.agency_id : []
